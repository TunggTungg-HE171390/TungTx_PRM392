...
import { View, Text, Button, StyleSheet, TouchableOpacity, FlatList, TextInput, ActivityIndicator, ScrollView, Alert } from 'react-native';
import { useSelector } from 'react-redux';
import { useDispatch } from 'react-redux';
import axios from 'axios'; import FontAwesome from "react-native-vector-icons/FontAwesome";
...
export default function Test({ navigation }) {
    ...
    const startQuiz = async () => {
        try {
            const res = await axios.post("http://192.168.1.38:9999/quiz/createQuiz", {
                title: title,
    ...
    const submitQuiz = async () => {
        ...
        try {
            const response = await axios.post(
                `http://192.168.1.38:9999/quiz/submit/${quizData._id}`,
                { answers }
            );
            ...
            <View style={styles.container}>
                ...
                <ScrollView style={styles.feedbackContainer}>
                    {Array.isArray(aiFeedback) ? (
                        ...
                        <Text style={styles.aiFeedback}>{aiFeedback ?? "Không có nhận xét từ AI"}</Text>
                    )}
                </ScrollView>
                <TouchableOpacity style={styles.buttonWrapper}>
                    <Button
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <View style={styles.questionContainer}>
                        <Text style={styles.text}>Câu {index + 1}: {item.questionId.content}</Text>
                        {item.questionId.options.length > 0 ? (
                            <FlatList
                                ...
                                renderItem={({ item: option }) => (
                                    <TouchableOpacity
                                        style={[styles.button, item.answers && item.answers.includes(option) && styles.selectedAnswer]} // Đánh dấu đáp án đã chọn
